...
const TestTasks = () => {
  ...
  const handleGeneratePerformanceAnalysis = async (task) => {
    ...
    if (task.execution_task_type !== 'performance') {
      message.warning('只有性能任务才能生成性能瓶颈分析报告');
      return;
    }
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (status, record) => (
        <Space>
          <Tag className={`status-tag ${status === 'completed' ? 'completed' : status === 'running' ? 'generating' : status === 'failed' ? 'failed' : 'active'}`}>
            {getStatusText(status)}
          </Tag>
          {status === 'running' && record.progress !== undefined && (
            <Progress
              className="cool-progress"
      ...
      render: (_, record) => {
        if (record.total_cases) {
          return (
            ...
            >
              <span style={{ whiteSpace: 'nowrap', flexShrink: 0 }}>总计: {record.total_cases}</span>
              <Tag 
                color="green" 
                style={{ 
                  whiteSpace: 'nowrap',
  ...
  // 计算统计数据
  const getStats = () => {
    const scenarioCount = groupedTasks.scenario?.length || 0;
    const interfaceCount = groupedTasks.interface?.length || 0;
    const performanceCount = groupedTasks.performance?.length || 0;
    const totalCount = scenarioCount + interfaceCount + performanceCount;
    
    return { scenarioCount, interfaceCount, performanceCount, totalCount };
  };
    ...
    <div className="test-cases-container">
      ...
      <div className="tabs-container">
        <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          <h2 style={{ margin: 0, fontSize: 20, fontWeight: 600, background: 'linear-gradient(135deg, #667eea 0%, #764ba2 100%)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>执行任务管理</h2>
          <Button className="cool-button cool-button-primary" type="primary" icon={<PlusOutlined />} onClick={handleAdd}>
            新建任务
          </Button>
        </div>
            ...
            items={[
              {
                ...
                label: (
                  <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    ...
                    场景任务 ({groupedTasks.scenario?.length || 0})
                  </span>
                ),
              },
              {
                ...
                label: (
                  <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    ...
                    接口任务 ({groupedTasks.interface?.length || 0})
                  </span>
                ),
              },
              {
                ...
                label: (
                  <span style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
                    ...
                    性能任务 ({groupedTasks.performance?.length || 0})
                  </span>
                ),
              },
            ]}
      ...
      >
        <Form form={form} layout="vertical">
          ...
          <Form.Item
            ...
            label="执行任务类型"
            rules={[{ required: true, message: '请选择执行任务类型' }]}
          >
            <Select
              placeholder="选择执行任务类型"
              onChange={() => {
            ...
            <TabPane tab="基本信息" key="basic">
              <Descriptions column={1} bordered>
                ...
                <Descriptions.Item label="状态">
                  <Tag className={`status-tag ${selectedTask.status === 'completed' ? 'completed' : selectedTask.status === 'running' ? 'generating' : selectedTask.status === 'failed' ? 'failed' : 'active'}`}>
                    {getStatusText(selectedTask.status)}
                  </Tag>
                </Descriptions.Item>
                <Descriptions.Item label="进度">
                  {selectedTask.progress !== undefined ? (
                    <Progress className="cool-progress" percent={selectedTask.progress} />
                  ) : '-'}
                </Descriptions.Item>
            ...
            <TabPane tab="进度统计" key="progress">
              {selectedTask.total_cases ? (
                ...
                <div className="cool-empty">
                  ...
                  <div className="cool-empty-text">暂无进度数据</div>
                </div>
              )}
            </TabPane>
          </Tabs>
