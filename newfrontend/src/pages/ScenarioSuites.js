...
import { Button, Modal, Table, Tag, message, Space, Descriptions, Popconfirm, Progress, Badge, Collapse, Checkbox, Spin } from 'antd';
import { EyeOutlined, PlusOutlined, DeleteOutlined, EditOutlined, PlusCircleOutlined, ArrowUpOutlined, ArrowDownOutlined, UnorderedListOutlined } from '@ant-design/icons';
...
const ScenarioSuites = () => {
  ...
  const fetchSuites = async () => {
    ...
    try {
      ...
      console.error('获取场景组合失败', error);
      const errorMsg = getErrorMessage(error);
      message.error('获取场景组合失败: ' + errorMsg);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGenerateFromDependencies = async () => {
    setGenerating(true);
    try {
      const response = await client.post(`/api/suites/generate/${id}`);
      message.success(response.message || `成功生成 ${response.stored_count || 0} 个场景组合`);
      
      // 刷新列表
      await fetchSuites();
    } catch (error) {
      console.error('生成场景组合失败', error);
      const errorMsg = getErrorMessage(error);
      message.error('生成场景组合失败: ' + errorMsg);
    } finally {
      setGenerating(false);
    }
  };
    ...
    <div style={{ padding: '24px' }}>
      ...
      <div className="stats-bar">
        <div className="stat-item">
          ...
          <span className="stat-value">{suites.length}</span>
          <span className="stat-label">个用例集</span>
        </div>
        <div className="stat-item">
          <span className="stat-label">接口总数:</span>
          <span className="stat-value">
            {suites.reduce((sum, suite) => sum + (suite.test_case_count || 0), 0)}
          </span>
        </div>
      </div>
      ...
      <Spin spinning={loading}>
        {suites.length === 0 && !loading && (
          <div style={{ textAlign: 'center', padding: '40px 0', color: '#999', background: '#fff', borderRadius: '6px' }}>
            <p style={{ fontSize: 16, marginBottom: 8 }}>暂无场景组合</p>
            <p style={{ fontSize: 14 }}>请先运行"接口依赖分析"生成场景用例集</p>
          </div>
        )}
        <div>
          {suites.map((suite) => {
            const isSelected = selectedSuiteIds.includes(suite.id);
            const isGenerating = generatingTestCases[`${suite.id}_pytest_scenario`];
            const tags = suite.tags ? suite.tags.split(',').map(t => t.trim()) : [];
            ...
            return (
              ...
              >
                <div className="card-row">
                  ...
                  />
                  <UnorderedListOutlined style={{ fontSize: '20px', color: '#1890ff', flexShrink: 0 }} />
                  <div className="card-content">
                    <div className="card-title">{suite.name || '未命名用例集'}</div>
                    <div className="card-url" style={{ fontFamily: 'inherit' }}>
                      {suite.description || '无描述'} • {suite.test_case_count || 0} 个接口
                    </div>
                  </div>
                  ...
                  <div style={{ display: 'flex', gap: '8px', flexShrink: 0 }}>
                    <Button
                      ...
                      onClick={(e) => {
                        e.stopPropagation();
                        handleGenerateTestCases(suite.id, 'pytest', 'scenario');
                      }}
                      loading={isGenerating}
                    >
                      生成场景用例
                    </Button>
                    <span className="card-action" onClick={(e) => {
                      e.stopPropagation();
                      handleViewDetail(suite);
                    }}>
                      查看详情 →
                    </span>
                  </div>
