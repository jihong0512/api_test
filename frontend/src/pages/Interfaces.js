...
import { 
  ...
  Card, Descriptions, Popconfirm, Tabs, Divider, Badge, InputNumber, Tooltip, Progress, Radio
} from 'antd';
import { 
  EditOutlined, DeleteOutlined, BugOutlined, EyeOutlined, 
  CheckCircleOutlined, CloseCircleOutlined, ClockCircleOutlined,
  BranchesOutlined, FileTextOutlined, ThunderboltOutlined
...
const Interfaces = () => {
  ...
  const fetchInterfaces = async () => {
    ...
    try {
      ...
      message.error('获取接口列表失败: ' + getErrorMessage(error));
    } finally {
      setLoading(false);
    }
  };
  ...
  const columns = [
    {
      ...
      render: (text, record) => (
        <Space>
          <Tag color={record.deprecated ? 'default' : 'blue'}>
            {record.method}
          </Tag>
          <span>{text || record.service || '未命名接口'}</span>
        </Space>
      )
    },
    {
    ...
    },
    {
      ...
      key: 'version',
      width: 100,
      render: (version) => version ? <Tag color="blue">{version}</Tag> : '-'
    },
    {
      title: '状态码',
    ...
    },
    {
      ...
      render: (headers) => {
        ...
        return (
          <Tooltip placement="topLeft" title={headersStr}>
            <div style={{ maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {headersStr.length > 50 ? headersStr.substring(0, 50) + '...' : headersStr}
            </div>
          </Tooltip>
        );
      }
    },
    {
      ...
      render: (body) => {
        ...
        return (
          <Tooltip placement="topLeft" title={bodyStr.length > 200 ? bodyStr.substring(0, 200) + '...' : bodyStr}>
            <div style={{ maxWidth: 200, overflow: 'hidden', textOverflow: 'ellipsis' }}>
              {bodyStr.length > 50 ? bodyStr.substring(0, 50) + '...' : bodyStr}
            </div>
          </Tooltip>
        );
      }
    ...
    <div style={{ padding: '24px' }}>
      <Card>
        <div style={{ marginBottom: 16 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: selectedRowKeys.length > 0 ? 12 : 0 }}>
            <h2 style={{ margin: 0 }}>接口列表</h2>
            <Badge count={interfaces.length} showZero>
              <span style={{ fontSize: 14, color: '#666' }}>共 {interfaces.length} 个接口</span>
            </Badge>
          </div>
          {selectedRowKeys.length > 0 && (
            <div style={{ display: 'flex', justifyContent: 'flex-end' }}>
              <Space>
                ...
                >
                  分析选中接口依赖 ({selectedRowKeys.length})
                </Button>
              </Space>
            </div>
          )}
        ...
        {interfaces.length === 0 && !loading && (
          <div style={{ textAlign: 'center', padding: '40px 0', color: '#999' }}>
            ...
            <p style={{ fontSize: 14 }}>请前往"文档管理"页面上传并解析接口文档（如：ks接口列表.md）</p>
          </div>
        )}
        <Table
          columns={columns}
        ...
        <div style={{ padding: '16px 0' }}>
          <div style={{ marginBottom: 24 }}>
            ...
            <div style={{ fontSize: 12, color: '#999' }}>
              测试用例将通过异步任务生成，生成完成后可在"测试用例"页面查看
            </div>
          </div>
