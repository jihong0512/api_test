  ...
  const fetchGraphData = async () => {
    ...
    try {
      ...
      if (data && data.neo4j_available === false) {
        ...
        message.warning('Neo4j服务不可用，知识图谱功能受限。正在使用数据库元数据作为备选方案。', 5);
      ...
      console.error('获取Neo4j知识图谱失败，尝试从元数据获取', error);
      // 如果Neo4j数据不可用，从元数据获取
      message.info('知识图谱功能受限，使用数据库元数据作为备选方案', 3);
      await fetchGraphDataFromMetadata();
    } finally {
      setLoading(false);
    }
  ...
  const fetchGraphDataFromMetadata = async () => {
    try {
      ...
      if (nodes.length === 0 && edges.length === 0) {
        message.warning('暂无知识图谱数据，请先分析数据库元数据');
      }
    } catch (err) {
      console.error('从元数据获取知识图谱失败', err);
      message.error('获取知识图谱失败: ' + (err.response?.data?.detail || err.message));
      setGraphData({ nodes: [], edges: [] });
    }
  };
          ...
          {graphData ? (
            graphData.nodes && graphData.nodes.length > 0 ? (
              ...
              <div style={{ textAlign: 'center', padding: 50 }}>
                {graphData.error ? (
                  <div>
                    <p style={{ color: '#ff4d4f' }}>获取知识图谱失败</p>
