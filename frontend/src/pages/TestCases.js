      ...
      return (
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
          <Space>
            ...
            {status && (
              <Tag color={status === 'active' || status === 'completed' ? 'green' : 'default'}>
                {status === 'active' ? '活跃' : status === 'completed' ? '已完成' : status === 'generating' ? '生成中' : status}
              </Tag>
            )}
  ...
  return (
    <div>
      <div style={{ marginBottom: 16, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <h2>测试用例管理</h2>
        <Space>
          <Button icon={<ReloadOutlined />} onClick={fetchTestCases}>
            刷新
          </Button>
          <Button 
            type="primary" 
              ...
              children: (
                <Spin spinning={loading}>
                  {treeData.length > 0 ? (
                    ...
                    <Empty description="暂无接口场景用例，请先创建" />
                  )}
                </Spin>
              )
            },
              ...
              children: (
                <Spin spinning={loading}>
                  {treeData.length > 0 ? (
                    ...
                    <Empty description="暂无接口测试用例，请先创建" />
                  )}
                </Spin>
              )
            },
              ...
              children: (
                <Spin spinning={loading}>
                  {treeData.length > 0 ? (
                    ...
                    <Empty description="暂无性能测试用例，请先创建" />
                  )}
                </Spin>
              )
            }
